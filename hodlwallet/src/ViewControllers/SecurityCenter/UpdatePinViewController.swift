...
class UpdatePinViewController : UIViewController, Subscriber {
    ...
    //MARK: - Private
    private let header = UILabel.wrapping(font: .customBold(size: 26.0), color: .whiteTint)
    private let instruction = UILabel.wrapping(font: .customBody(size: 14.0), color: .whiteTint)
    private let caption = UILabel.wrapping(font: .customBody(size: 13.0), color: .grayTextTint)
